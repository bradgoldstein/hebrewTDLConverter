...
class advRowConverter(rowConverter):
        ...
        def printAllAdvs(lexicon, fileName):#LHS: no need to check for punctuation here anymore
                ...
                for tup in advRowConverter.allAdvs:
                        ...
                        if not wantPunctuation and has_punct or wantPunctuation and not has_punct:
                                continue'''#LHS: not needed anymore
        ...
        def addToDictionary(self):
                ...
                if (stem, pred, DEFAULT_LEXICAL_POINTER) not in advRowConverter.allAdvs:#LHS - avoid adding entries that were in the pre-existing lexicon
                        advRowConverter.allAdvs.add((stem, pred, self.getLexicalPointer()))#LHS
